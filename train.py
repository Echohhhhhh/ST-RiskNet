...
from model.STRiskNet import ST_RiskNet
...
def training(net,
    ...
    for epoch in range(1,training_epoch+1):
        ...
        for train_feature,target_time,gragh_feature,train_label in train_loader:
            ...
            l = mask_loss(net(train_feature,target_time,gragh_feature,road_adj,grid_node_map),train_label,risk_mask,data_type=data_type)
            trainer.zero_grad()
            l.backward()
            trainer.step()
        ...
        val_loss = compute_loss(net,val_loader,risk_mask,road_adj,grid_node_map,global_step-1,device,data_type)
        ...
        if epoch == 1 or val_loss < early_stop.best_score:
            test_rmse,test_recall,test_map,test_inverse_trans_pre,test_inverse_trans_label = \
                        predict_and_evaluate(net,test_loader,risk_mask,road_adj,grid_node_map,global_step-1,scaler,device)
           
            high_test_rmse,high_test_recall,high_test_map,_,_ = \
                        predict_and_evaluate(net,high_test_loader,risk_mask,road_adj,grid_node_map,global_step-1,scaler,device)
...
def main(config):
    ...
    ST_RiskNet_Model= ST_RiskNet(train_data_shape[2],num_of_gru_layers,seq_len,pre_len,
                    gru_hidden_size,time_shape[1],graph_feature_shape[2],
                    nums_of_filter,north_south_map,west_east_map)
    if torch.cuda.device_count() > 1:
        print("Let's use", torch.cuda.device_count(), "GPUs!",flush=True)
        ST_RiskNet_Model = nn.DataParallel(ST_RiskNet_Model)
    ST_RiskNet_Model.to(device)
    print(ST_RiskNet_Model)

    num_of_parameters = 0
    for name,parameters in ST_RiskNet_Model.named_parameters():
        num_of_parameters += np.prod(parameters.shape)
    print("Number of Parameters: {}".format(num_of_parameters), flush=True)


    trainer = optim.Adam(ST_RiskNet_Model.parameters(), lr=learning_rate)
