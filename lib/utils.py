...
@torch.no_grad()
def compute_loss(net,dataloader,risk_mask,road_adj,
    ...
    for feature,target_time,graph_feature,label in dataloader:
        ...
        l = mask_loss(net(feature,target_time,graph_feature,road_adj,grid_node_map),label,risk_mask,data_type)#l的shape：(1,)
        temp.append(l.cpu().item())
    loss_mean = sum(temp) / len(temp)
    return loss_mean

@torch.no_grad()
def predict_and_evaluate(net,dataloader,risk_mask,road_adj,
    ...
    for feature,target_time,graph_feature,label in dataloader:
        feature,target_time,graph_feature,label = feature.to(device),target_time.to(device),graph_feature.to(device),label.to(device)
        prediction_list.append(net(feature,target_time,graph_feature,road_adj,grid_node_map).cpu().numpy())
